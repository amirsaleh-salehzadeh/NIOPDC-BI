 ...
 */
public class AIPXMLA_SOAP implements OlapDiscoverer {
  ...
  //aip adl
  static final String SSAS_URI = "http://schemas.microsoft.com/analysisservices/2003/engine";
  static final String EMPTY_URI = "urn:schemas-microsoft-com:xml-analysis:empty";

  static Logger logger = Logger.getLogger(AIPXMLA_SOAP.class);
   ...
   * @param password  
   */
  public AIPXMLA_SOAP(String uri, String user, String password) throws OlapException {
   ...
   * @param dataSource
   */
  public AIPXMLA_SOAP(String uri, String user, String password, String dataSource)
   ...
   * @param provider
   */
  public AIPXMLA_SOAP(String uri, String user, String password, int newProvider) throws OlapException {
    ...
    Rowhandler rh = new Rowhandler() {
      public void handleRow(SOAPElement eRow, SOAPEnvelope envelope) {
        ...
        while (it.hasNext()) {
          ...
          //System.out.println("AIPXMLA_SOAP.handleRow()");
          //AIPUtil.printObject(e);
  ...
  public List discoverLev(String cat, String cube, String dimension, String hier)
    ...
    if (dimension != null)
      rHash.put("DIMENSION_UNIQUE_NAME", dimension);
    if (hier != null)
      rHash.put("HIERARCHY_UNIQUE_NAME", hier);
...
*/      
   ...
   * @throws OlapException
   */
  public void discover(String request, URL url, Map restrictions, Map properties, Rowhandler rh)
      throws OlapException {
...
//      System.out.println("AIPXMLA_SOAP.discover()----------------------------------");
//	    try {
 ...
 * aip adl
 */
	  public List discoverCat(String catalog) throws OlapException {
		    final List cats = new ArrayList();

		    // restrictions
		    HashMap rHash = new HashMap(); // empty
		    rHash.put("CATALOG_NAME", catalog);
	...
	public List discoverRole(String catalog)throws OlapException {
		List<String> roles=new ArrayList<String>();
	
		List<XMLA_OlapItem> cats = discoverCat(catalog);
		for(XMLA_OlapItem item : cats){
			if(catalog.equalsIgnoreCase( item.getName() ) ){
				String[] stRoles = NVL.getString( item.getProperty("ROLES")).split(",");
				for (String role : stRoles) {
	...
	public List discoverUserRoles(String cat, String user) throws OlapException {
	    final List<String> roles = new ArrayList();
	    final String _user = NVL.getString(user);
	    ...
	    Rowhandler rh = new Rowhandler() {
	      public void handleRow(SOAPElement eRow, SOAPEnvelope envelope) {
	    	  ...
	    	  try {
	        	  ...
	        	  SOAPElement metaData = selectSingleNode(eRow, envelope.createName("METADATA", "xars", ROWS_URI));
	        	  if(metaData!=null){
	        	        Iterator itRole = metaData.getChildElements();
	        	        while (itRole.hasNext()) {
	        	        	SOAPElement role = (SOAPElement) itRole.next();
	
//	        	        	System.out.println("role="+role.getElementName().getLocalName());
        	        		SOAPElement roleName2 = selectSingleNode2(role, "Name");
            	        	System.out.println("roleName="+roleName2.getValue());
	        	        	
	        	        	SOAPElement members =  selectSingleNode2(role, "Members");
	        	        	if(members!=null){
	        	        		boolean found = false;
	                	        Iterator itMember = members.getChildElements();
	                	        while (itMember.hasNext() && !found) {
	                	        	SOAPElement member = (SOAPElement) itMember.next();
	
//	                	        	System.out.println("member="+member.getElementName().getLocalName());
	                	        	
	                	        	if("Member".equalsIgnoreCase(member.getElementName().getLocalName())){
	                	        		
	                    	        	SOAPElement memberName =  selectSingleNode2(member, "Name");
	                    	        	
	                    	        	System.out.println("memberName="+memberName.getValue());
	                    	        	
	                    	        	if(memberName!=null  && _user.equalsIgnoreCase(memberName.getValue())){//find user
	                    	        		
	                    	        		SOAPElement roleName = selectSingleNode2(role, "Name");
	    ...
	    List<String> stRoles = discoverRole(cat);
	    for (int i = 0; i < stRoles.size(); i++) {
	    	String role = stRoles.get(i);
	    	if(!"".equals(role)){
	    	
//	    		System.out.println("AIPXMLA_SOAP.discoverUserRoles():role="+role);
		        // restrictions
		        HashMap rHash = new HashMap();
		        rHash.put("DatabaseID", cat);
		        rHash.put("RoleID", role);
		
		        discover("DISCOVER_XML_METADATA", url, rHash, pHash, rh);
	    	}
		}
	    logger.debug("DISCOVER_XML_METADATA: found " + roles.size());
//	    if (roles.size() == 0) {
//	        throw new OlapException("No metadata schema roles for catalog: " + cat + " and user: " + user);
//	    }
	    return roles;
	  }
	public List discoverRoleUsers(String cat, String role) throws OlapException {
		final List<String> users = new ArrayList<String>();
	    final String _role = NVL.getString(role);
	        	        ...
	        	        while (itRole.hasNext()) {
	        	        	...
	        	        	if(members!=null){
	                	        ...
	                	        while (itMember.hasNext()) {
	                	        	...
	                	        	if("Member".equalsIgnoreCase(member.getElementName().getLocalName())){	                	        		
	                    	        	SOAPElement memberName =  selectSingleNode2(member, "Name");
	                    	        	if(memberName!=null ){//find user
	                    	        		users.add(memberName.getValue());
	                    	        	}
        ...
        logger.debug("DISCOVER_XML_METADATA: found " + users.size());
//	    if (users.size() == 0) {
//	        throw new OlapException("No metadata schema role users for catalog: " + cat + " and role: " + role);
//	    }
	    return users;
	  }
		
	private SOAPElement selectSingleNode2(SOAPElement contextNode, String localName) {
//		System.out.println("AIPXMLA_SOAP.selectSingleNode2()");
		
	    Iterator it = contextNode.getChildElements();
	    while (it.hasNext()){
	    	SOAPElement elm = (SOAPElement) it.next();
//	    	System.out.println(elm.getElementName().getLocalName());
	    	if(localName.equalsIgnoreCase( elm.getElementName().getLocalName()) ){
	    		return elm;
	    	}
	    }
	    return null;
  }
	public static void printSOAPElement(SOAPElement elm){
		printSOAPElement(elm,0);
		System.out.println();
	}
	private static void printStringCount(String st, int count){
		for (int i = 0; i < count; i++) {
			System.out.print(st);
		}
	}
	private static void printSOAPElement(SOAPElement elm,int tabcount){
		System.out.println();
		printStringCount("\t", tabcount);
		System.out.print( "<"+elm.getElementName().getQualifiedName() );
		Iterator itAttr = elm.getAllAttributes();
		while(itAttr.hasNext()){
			Name attr = (Name) itAttr.next();
			System.out.print(" "+attr.getLocalName()+"='"+elm.getAttributeValue(attr)+"'");
		}
		System.out.print(">");
		Iterator itChild = elm.getChildElements();
		while(itChild.hasNext()){
			Object o = itChild.next();
			if(o instanceof SOAPElement){
				SOAPElement child = (SOAPElement) o;
				printSOAPElement(child,tabcount+1);
			}else if(o instanceof TextImpl){
				TextImpl t = (TextImpl) o;
				System.out.println();
				printStringCount("\t", tabcount+1);
	...
	interface MsgBodyhandler {
		    void handleBody(SOAPElement eParent, SOAPEnvelope envelope)throws SOAPException;
		  }


	  private SOAPElement findExecRoot2(SOAPMessage reply) throws SOAPException, OlapException {
		    SOAPPart sp = reply.getSOAPPart();
		    SOAPEnvelope envelope = sp.getEnvelope();
		    SOAPBody body = envelope.getBody();
		    ...
		    //name = envelope.createName("root", "", EMPTY_URI);
		    SOAPElement eRoot = selectSingleNode2(eReturn, "root");
		    if (eRoot == null) { throw new OlapException("Excecute result has no root element"); }
		    return eRoot;
		  }

	  private void execute(MsgBodyhandler handler)
      throws OlapException {
    ...
    try {
      ...
      handler.handleBody(eCommand, envelope);
...
//      printSOAPElement(eCreate);
      System.out.println("AIPXMLA_SOAP.execute()====================================");
      try { msg.writeTo(System.out); } catch (IOException e) {e.printStackTrace();}
      
      if (logger.isDebugEnabled()) {
          logger.debug("Execute xmla command ");
          //reply.getSOAPPart().getContent().
          logSoapMsg(msg);
        }
      // run the call
      ...
      SOAPElement eRoot = findExecRoot2(reply);
	...
	public void createRole(String catalog,String roleName,String[] membership)throws OlapException{
		final String _catalog = catalog;
		final String _roleName = roleName;
		final String[] _membership = membership;
		
		MsgBodyhandler mbh = new MsgBodyhandler(){
			public void handleBody(SOAPElement parent, SOAPEnvelope envelope) throws SOAPException{
				
		      Name nCreate = envelope.createName("Create","",SSAS_URI);
		      SOAPElement eCreate = parent.addChildElement(nCreate);
		
		      Iterator itAttr=null;
		      
		      Name nParentObject = envelope.createName("ParentObject","",SSAS_URI);
		      SOAPElement eParentObject = eCreate.addChildElement(nParentObject);
		      
		      Name nDatabaseID = envelope.createName("DatabaseID","",SSAS_URI);
		      SOAPElement eDatabaseID = eParentObject.addChildElement(nDatabaseID);
		      
		      eDatabaseID.addTextNode(_catalog);
		      
		      Name nObjectDefinition = envelope.createName("ObjectDefinition","",SSAS_URI);
		      SOAPElement eObjectDefinition = eCreate.addChildElement(nObjectDefinition);
		
		      Name nRole = envelope.createName("Role","",SSAS_URI);
		      SOAPElement eRole = eObjectDefinition.addChildElement(nRole);
		
		      Name nName = envelope.createName("Name","",SSAS_URI);
		      SOAPElement eName = eRole.addChildElement(nName);
		      eName.addTextNode(_roleName);
		      
		      Name nMembers = envelope.createName("Members","",SSAS_URI);
		      SOAPElement eMembers = eRole.addChildElement(nMembers);
		
		      for (int i = 0; i < _membership.length; i++) {
			      Name nMember = envelope.createName("Member","",SSAS_URI);
			      SOAPElement eMember = eMembers.addChildElement(nMember);
			
			      Name nMemberName = envelope.createName("Name","",SSAS_URI);
			      SOAPElement eMemberName = eMember.addChildElement(nMemberName);
			      eMemberName.addTextNode(_membership[i]);
		      }
				//printSOAPElement(parent);
			}
		};
	...
	public void addRoleMemberships(String catalog,String roleName,String[] membership)throws OlapException{
		final String _catalog = catalog;
		final String _roleName = roleName;
		
		final List _allMembership = discoverRoleUsers(catalog, roleName);
		for (int i = 0; i < membership.length; i++) {
			if( ! _allMembership.contains(membership[i]) ){
				_allMembership.add(membership[i]);
			}
		}
		...
		MsgBodyhandler mbh = new MsgBodyhandler(){
			public void handleBody(SOAPElement parent, SOAPEnvelope envelope) throws SOAPException{
		      ...
		      Name nCreate = envelope.createName("Alter","",SSAS_URI);
		      SOAPElement eCreate = parent.addChildElement(nCreate);
		
		      Iterator itAttr=null;
		      
		      Name nParentObject = envelope.createName("Object","",SSAS_URI);
		      SOAPElement eParentObject = eCreate.addChildElement(nParentObject);
		      ...
		      Name nRoleID = envelope.createName("RoleID","",SSAS_URI);
		      SOAPElement eRoleID = eParentObject.addChildElement(nRoleID);
		      eRoleID.addTextNode(_roleName);
		      ...
		      for (int i = 0; i < _allMembership.size(); i++) {
			      Name nMember = envelope.createName("Member","",SSAS_URI);
			      SOAPElement eMember = eMembers.addChildElement(nMember);
			
			      Name nMemberName = envelope.createName("Name","",SSAS_URI);
			      SOAPElement eMemberName = eMember.addChildElement(nMemberName);
			      eMemberName.addTextNode(NVL.getString(_allMembership.get(i)));
		      }
				//printSOAPElement(parent);
			}
		};
	...
	public void delRoleMemberships(String catalog,String roleName,String[] membership)throws OlapException{
		final String _catalog = catalog;
		final String _roleName = roleName;
		...
		for (int i = 0; i < membership.length; i++) {
			if( _allMembership.contains(membership[i]) ){
				_allMembership.remove(membership[i]);
			}
		}
	...
	public void addUserRoles(String catalog,String user,String[] roles)throws OlapException{
		String[] membership={user};
		for (int i = 0; i < roles.length; i++) {
			if( !"".equals( NVL.getString( roles[i] ) ) ){
				addRoleMemberships(catalog, roles[i], membership);
			}
		}
	}

	public void updateUserRoles(String catalog,String user,String[] roles)throws OlapException{
		List<String> oldRoles = this.discoverUserRoles(catalog, user);
		Hashtable htOldRoles = new Hashtable();
		for (String oldRole : oldRoles) {
			htOldRoles.put(oldRole, oldRole);
		}

		ArrayList newRoles = new ArrayList();
		for (int i=roles.length-1;i>=0;i--) {
			if(htOldRoles.containsKey(roles[i])){
				htOldRoles.remove(roles[i]);
			}else{
		...
		//del
		for (Object role: htOldRoles.values()) {
			if( !"".equals( NVL.getString( role ) ) ){
				delRoleMemberships(catalog, NVL.getString( role ), membership);
			}
		}
	}
